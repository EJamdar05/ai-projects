...
class State {
	...
	/*
	 * goalTest function will take a state and determine if the amount of husbands
...
public class Node {
	...
	/*
	 * stack will keep track of nodes that are expanded upon explored will help make
	 * sure that we do not end up in loopy paths because it will keep track of
	...
	public static void main(String[] args) {
		...
		Node N1 = new Node(S1, null, "Start                            "); // adding the first node to explored (which has init state)
		explored.add(S1);
		stack.add(N1);
		...
		while (stack.size() > 0) {
			// get node from stack and then pop it from the list
			Node parentN = stack.get(stack.size() - 1);
			stack.remove(stack.size() - 1);

			// if no more husbands and wives exist on the left side, then solution is found
			if (parentN.getState().goalTest(parentN)) { // goal test
			 ...
			 * want to add that to the explored array and also our stack so that we can
			 * observe the other potential nodes
			 */
			for (int i = 0; i < States.size(); i++) {
				if (!isExplored(States.get(i))) {
					explored.add(States.get(i));
					stack.add(new Node(States.get(i), parentN, States.get(i).getAction()));
				}
			}
		}
